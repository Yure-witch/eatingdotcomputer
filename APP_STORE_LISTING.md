# App Store listing — eating.computer

Copy-paste into App Store Connect. Written to appeal to art/design students, not
to read like a corporate LMS. Character limits noted; all fields fit.

---

## App Name  *(30 char max)*
```
eating.computer
```

## Subtitle  *(30 char max)*
```
Your class, but it's alive
```
Alternates: `Class chat with a pulse` · `Where the class hangs out` · `Studio, chat & tools`

## Promotional text  *(170 char max — editable anytime without review)*
```
Class chat, assignments, files, and a week-by-week roadmap in one place. The Lab adds kinetic type and 3D text GIF tools you can export straight into your work.
```
*160/170.* Says what is in the app and stops. The previous version ("doesn't
feel like homework… wild animated stickers… made for how you actually work")
argued with the reader and made claims nothing backs up, spending characters
that could name a feature instead.

Alternates:
- `Everything for a studio class in one place: real-time chat, assignments, files, and a week-by-week roadmap. Plus in-app tools for kinetic type and 3D text GIFs.`
- `Real-time class chat with reactions, replies, and threads. Assignments and a week-by-week roadmap. A Lab for making kinetic type and 3D lettering you can export.`

## Keywords  *(100 char max, comma-separated, no spaces after commas)*
```
class,chat,art school,studio,assignments,stickers,gif,emoji,students,creative,critique,messaging
```

## Description  *(4000 char max)*
```
eating.computer is the home base for your class — a place that feels less like a
learning portal and more like the group chat you actually want to be in.

Built for studio classes and the people in them.

CHAT THAT ACTUALLY EXPRESSES SOMETHING
Real-time channels and DMs with reactions, replies, threads, and mentions. Send
animated stickers, emoji mixes, custom emotes, and text that moves — bold
colors, kinetic effects, the works. It's messaging that matches how you think.

EVERYTHING FOR THE CLASS, IN ONE PLACE
• Assignments and a week-by-week roadmap, with link / image / video submissions
• Files — everything shared in the class, easy to find
• A profile you can actually make your own

MAKE THINGS, RIGHT HERE
The Lab has creative tools built in: Kinetic Type for mesmerizing title
animations, and Text GIFs for 3D chrome / gold / glass lettering you can drop
straight into a deck. Export a GIF, drag it into your slides, done.

GEMMA, YOUR CLASS COMPANION
Gemma quietly keeps you on track — a friendly recap of what happened, gentle
reminders about what's due, and a spark of inspiration tuned to what you're into.
No spam, no nagging.

STAYS OUT OF YOUR WAY
Push notifications when something needs you. Works on your phone and your laptop,
in perfect sync. Fast, expressive, and made for how studio classes really run.

For students in a class on eating.computer. Sign in with your class account to
join.
```

## What's New (version notes for 1.0)
```
Hello, App Store 👋 The full eating.computer experience, now native on iOS —
with push notifications so you never miss a message, mention, or deadline.
```

## Support URL
```
https://www.eating.computer
```

## Marketing URL (optional)
```
https://www.eating.computer
```

## Category
- Primary: **Education**
- Secondary: **Social Networking**

## Age Rating
Expect **12+** (user-generated content / unrestricted messaging within a class).
Answer the questionnaire honestly: users can post text and images to each other.

## App Privacy (must match ios/App/App/PrivacyInfo.xcprivacy)
Data collected, all **linked to the user**, all **App Functionality**, **none
used for tracking**:
- Contact Info → Email Address, Name
- User Content → Photos or Videos, Other User Content (messages)

## Screenshots
Use the rendered set (see the App Store gallery artifact). Required sizes:
- **6.9"/6.7" iPhone** — 1290 × 2796 (portrait). This is the one Apple shows.
- **6.5" iPhone** — 1242 × 2688 (portrait).
Upload 3–6, ordered strongest first: Chat/expression → Text GIFs/Lab →
Assignments/Roadmap → Gemma → Profile.

## Review notes (paste into "App Review Information → Notes")
```
eating.computer is the platform for a college studio class. Core native value:
push notifications (APNs), the native keyboard-aware compose bar, and the
in-app creative tools (Kinetic Type, 3D Text GIFs). A demo account is provided
below so review can see the class chat, assignments, and tools without an
enrollment.
```
Provide a **demo account** (username + password) in the App Review section — an
account already approved into a class so reviewers land straight in the app.
```
