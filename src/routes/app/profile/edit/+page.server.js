...
export const actions = {
	default: async ({ request, locals }) => {
		...
		// Avatar choice. 'gen' wipes avatar_value back to NULL so the
		// component falls through to the gradient + initial. 'photo'
		// uploads a fresh file to R2 (when one was attached) and stores
		// its public URL. 'expr' persists the inline token.
		...
		if (avatarKind === 'expr') {
			...
			if (file && typeof file !== 'string' && file.size > 0) {
				...
				// "photo" picked but no new file submitted — keep the
				// existing URL stored on the user row.
		...
		await db.execute({
			sql: 'UPDATE users SET name = ?, pronouns = ?, bio = ?, website = ?, year = ?, school = ?, focus = ?, avatar_kind = ?, avatar_value = ? WHERE id = ?',
			args: [
