...
<style>
	...
	@media (max-width: 640px) {
		...
		/* Grabber drag = transforms only (ExpressionPicker's setDragExtra):
		   during html.expr-grow-dragging the sheet and the compose ride
		   --expr-grow-drag on the compositor; --expr-grow (real layout)
		   commits once on release. Scoped to the transient class so no
		   permanent containing blocks are created for the fixed pickers. */
