<script>
	...
	function markRead() {
		set(ref(db, `lastRead/${data.currentUser.id}/${convId}`), Date.now());
	}
	...
	onMount(() => {
		...
		onValue(reactionsRef, (snap) => {
			...
			// This prevents Firebase firing before a write propagates from wiping optimistic updates
			const merged = {};
	...
	async function send() {
		...
		try {
			await fetch('/api/chat', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ content: content || attSnap?.filename || '', channelId: convId, reply_to: replySnap, attachment: attSnap })
			});
