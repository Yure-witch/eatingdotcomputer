<script>
	...
	function processCodeBlocks(text) {
		...
		// If we only have inline code and no blocks, return null to let normal flow handle it
		// (inline code is simpler and handled below)
	...
	function contentHtml(text, split = true) {
		if (!text) return '';
		// Handle code blocks first (before FX/emoji processing)
		...
		if (codeParts) {
			return codeParts.map((p, i) => {
				...
				// Trim whitespace-only gaps between code blocks
		...
		if (text.includes('`')) {
			...
			if (inlined !== text) {
				// Process remaining text parts for emoji/fx but preserve the <code> tags
				// Split by inline code, process each non-code part, rejoin
				...
				return splitParts.map(part => {
					...
					// Process as normal text (remove the html we just created, process raw)
