<script>
	...
	onMount(() => {
		...
		onValue(reactionsRef, (snap) => {
			...
			// Must merge at the user level within each emoji key — a shallow spread at the
			// emoji level would let Firebase's partial data overwrite Turso's complete data.
			const merged = {};
