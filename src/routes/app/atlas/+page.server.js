...
export async function load({ locals, parent }) {
	...
	for (const row of rows) {
		...
		const assignment = { id: row.id, title: row.title, description: row.description, dueDate: row.due_date, acceptedTypes };
		if (isInstructor) { assignment.submissions = await getSubmissionsForAssignment(row.id); }
		else { assignment.mySubmission = await getStudentSubmission(row.id, userId); }
		byWeek[w].push(assignment);
	}
	const weeks = Object.keys(byWeek).map(Number).sort((a, b) => a - b).map((w) => ({ week: w, assignments: byWeek[w] }));
	...
	return { weeks, role: session.user.role, userId, classId, links, uploadedFiles, starredMessages };
}
